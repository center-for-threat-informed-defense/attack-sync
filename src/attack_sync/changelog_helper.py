...
class DiffStix(object):
    ...
    def load_domain(self, domain: str):
        ...
        for datastore_version in ["old", "new"]:
            ...
            if self.use_mitre_cti and datastore_version == "old":
                data_store = self.get_datastore_from_mitre_cti(
                    domain=domain, datastore_version=datastore_version
                )
            else:
    ...
    def get_datastore_from_mitre_cti(
        self, domain: str, datastore_version: str
    ) -> stix2.MemoryStore:
        ...
        attack_version = release_info.get_attack_version(
            domain=domain, stix_content=stix_response.content
        )
    ...
    def get_groupings(
        self, object_type: str, stix_objects: List, section: str, domain: str
    ...
    def placard(self, stix_object: dict, section: str, domain: str) -> str:
        ...
        if section == "deletions":
            ...
            if revoker.get("x_mitre_is_subtechnique"):
                ...
                placard_string = f"{stix_object['name']} (revoked by {parent_name}: [{revoker['name']}]({revoker_link}))"
                ...
                parent_name = parent_object.get("name", "ERROR NO PARENT")
                relative_url = get_relative_data_component_url(
                    datasource=parent_object, datacomponent=stix_object
                )
                revoker_link = f"{self.site_prefix}/{relative_url}"
                placard_string = f"{stix_object['name']} (revoked by {parent_name}: [{revoker['name']}]({revoker_link}))"
        ...
        version_string = get_placard_version_string(
            stix_object=stix_object, section=section
        )
    ...
    def get_markdown_section_data(self, groupings, section: str, domain: str) -> str:
        ...
        for grouping in groupings:
            if grouping["parentInSection"]:
                placard_string = self.placard(
                    stix_object=grouping["parent"], section=section, domain=domain
                )
                sectionString += f"* {placard_string}\n"

            for child in sorted(grouping["children"], key=lambda child: child["name"]):
                placard_string = self.placard(
                    stix_object=child, section=section, domain=domain
                )
                ...
                if grouping["parentInSection"]:
                    ...
                    sectionString += (
                        f"* {grouping['parent']['name']}: {placard_string}\n"
                    )
        ...
        for object_type in self.types:
            ...
            for domain in self.data["changes"][object_type]:
                ...
                if domain == "mobile-attack" and object_type == "datasource":
                    logger.debug(
                        "Skipping - ATT&CK for Mobile does not support data sources"
                    )
                    domains += "ATT&CK for Mobile does not support data sources\n\n"
                    continue
                domain_sections = ""
                for section, stix_objects in self.data["changes"][object_type][
                    domain
                ].items():
...
def has_subtechniques(
    stix_object: dict, subtechnique_relationships: List[dict]
) -> bool:
...
def get_placard_version_string(stix_object: dict, section: str) -> str:
    ...
    if section in ["additions", "deprecations", "revocations"]:
        ...
        if not version_increment_is_valid(
            old_version=None, new_version=str(object_version), section=section
        ):
            color = red
    ...
    # nothing needs to be added to this statement - it just needs to skip the 'else' clause
        ...
        if not version_increment_is_valid(
            old_version=old_version, new_version=str(object_version), section=section
        ):
            color = red
...
def version_increment_is_valid(
    old_version: str, new_version: str, section: str
) -> bool:
...
def get_relative_url_from_stix(stix_object: dict) -> Optional[str]:
    ...
    """
    is_subtechnique = stix_object["type"] == "attack-pattern" and stix_object.get(
...
def markdown_to_html(outfile: str, content: str, diffStix: DiffStix):
    ...
    if new_version:
        header = template.render(dark=false, title="heading")
...
def write_detailed_index(html_file_detailed: str, diffStix: DiffStix):
    ...
    if len(diffStix.domains) == 1:
        old_version = diffStix.data["old"][diffStix.domains[0]][
            "attack_release_version"
        ]
        new_version = diffStix.data["new"][diffStix.domains[0]][
            "attack_release_version"
        ]
    ...
    with open(html_file_detailed, "w") as file:
        ...
        if len(diffStix.domains) < 2:
            ...
            buttons = nav_buttons.render(domain=diffStix.domains[0])
        else:
            header = template.render(
                theme="light",
                oldVersion=old_version,
        ...
        for object_type, domain_data in diffStix.data["changes"].items():
            ...
            if object_type in diffStix.types:
                ...
                if sum(
                    ...
                    lines.append(
                        f"<h4>{diffStix.attack_type_to_title[object_type]}</h4>"
                    )
                    ...
                    for domain in diffStix.domains:
                        if len(diffStix.domains) > 1:
                            lines.append(
                                f"<h5>{diffStix.domain_to_domain_label[diffStix.domains[0]]} ATT&CK</h5>"
                            )
                        lines.append(
                            template.render(
        ...
        for object_type, domain_data in diffStix.data["changes"].items():
            ...
            if object_type in diffStix.types:
                ...
                if sum(
                    ...
                    lines.append(
                        f"<h2>{diffStix.attack_type_to_title[object_type]}</h2>"
                    )
                else:
                    continue
                ...
                for domain, change_types in domain_data.items():
                    ...
                    for change_type, change_data in change_types.items():
                        ...
                        datastore_version = (
                            "old" if change_type == "deletions" else "new"
                        )

                        if change_data:
                            # build accordion for each change
                            lines.append(
                                f'<h4 class="change-type" id="{domain}_{category}_{change_type}">{diffStix.section_headers[object_type][change_type]}</h4>'
                            )
                            lines.append(
                                f'<div class="accordion accordion-flush" id="accordionFlush-{domain}-{change_type}">'
                            )
                            index = 0
                        ...
                        for stix_object in change_data:
                            ...
                            if stix_object[
                                "type"
                            ] == "x-mitre-data-component" or stix_object.get(
                                "x_mitre_is_subtechnique"
                            ):
                                ...
                                if parent_object:
                                    ...
                                    logger.warning(
                                        f"[{stix_object['id']}] {attack_id} has no parent!"
                                    )
...
def get_new_changelog_md(
    ...
    if html_file_detailed:
        ...
        write_detailed_html_refactor(
            html_file_detailed=html_file_detailed, diffStix=diffStix
        )
